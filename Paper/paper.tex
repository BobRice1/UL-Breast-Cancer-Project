...
\begin{table}[t]
\centering
\caption{Adjusted Rand Index (ARI) between $k$-means cluster assignments and diagnostic labels as a function of the number of retained principal components. For each configuration, the reported ARI corresponds to the solution with minimum inertia over 30 random initialisations using $k$-means++ initialisation and a fixed random seed. ARI is reported \textit{post hoc} for evaluation only and was not used to select the number of retained PCs.}
\label{tab:ari}
\begin{ruledtabular}
\begin{tabular}{ccccc}
