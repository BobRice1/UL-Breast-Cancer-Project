...
\paragraph{Principal component analysis:} Dimensionality reduction was performed using principal component analysis (PCA), fit to the full feature matrix without access to diagnostic labels. PCA was chosen due to its interpretability and its ability to identify dominant correlated directions in linear feature spaces. The sample covariance matrix
\begin{equation}
...
\end{equation}
where $\lambda_{\ell}$ and $\mathbf{v}_{\ell}$ denotes the eigenvalues and corresponding eigenvectors, ordered such that $\lambda_{1} \geq \lambda_{2} \geq \dots$. Each data point $\mathbf{x}_i$ was projected onto the first $K$ principal components via
\begin{equation}
\mathbf{z}_i = V_K^{\mathsf{T}} \mathbf{x}_i ,
\end{equation}
where $V_K = [\mathbf{v}_1, \dots, \mathbf{v}_K]$. The number of retained components was selected based on the cumulative explained variance.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.48\linewidth]{pca_loadings_pc1.png}
\hfill
\includegraphics[width=0.48\linewidth]{pca_loadings_pc2.png}
...
\begin{table}[t]
\centering
\caption{Adjusted Rand Index (ARI) between k-means cluster assignments and diagnostic labels as a function of the number of retained principal components.}
\label{tab:ari}
\begin{ruledtabular}
\begin{tabular}{ccccc}
